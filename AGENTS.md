...
- **Codex `agent_message` remains typed through Responses state.** Preserve `id`, `author`, `recipient`, ordered `input_text`/`encrypted_content` parts, replay hashes, and item references. Only the final Responses-to-Chat lowering turns it into a provenance-labelled user boundary. `agent_message_encrypted_content: plaintext_compat` is a distinct, opt-in local-Codex policy; never infer it from `encrypted_reasoning` or enable it for opaque ciphertext.
