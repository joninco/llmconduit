...
- Dashboard (gap 05): upstream RESPONSE/ERROR-body capture is a SEPARATE opt-in gate from the debug UI — `LLMCONDUIT_DASHBOARD_CAPTURE_UPSTREAM_RESPONSE=1/true/yes` (env-only, never on `Config`); `--with-debug-ui` alone arms only REQUEST capture. The body lands on the LIVE `FlowRecord.upstream_response` only (`UpstreamResponseBody{bytes,truncated}`), NEVER on `SnapshotFlowSummary` (body-free invariant). It reflects the TURN's FINAL outcome: the leaf STAGES each failed attempt's body on the shared `ServingToken` (`set_pending_response_body`, last-writer-wins) at `dispatch_chat_stream`'s terminal-error sites via `capture_response_body` (the SAME capped/redacting `capture_capped_redacted` path — copies the already-read error `String`, never slices the 256 MiB buffer; `truncated = raw.len() > BODY_CAP`); a provider that SERVES the turn clears the pending body (`clear_pending_response_body` at the failover serve-success seam), and the L1 `TelemetryGuard` commits the token's pending body to the record at finalize. So a failed-over-then-served turn (A 500 → B 200) carries `None`, an all-fail turn carries the LAST attempt's body. An empty/non-JSON upstream body is recorded as the `[redacted: unparseable body N bytes]` marker (`Some`), distinct from `None` (capture off / no body / a later provider served). Projected onto the LIVE `/dashboard/api/flows/:id` detail as the optional `FlowDetailBody.upstream_response = {body, truncated}` (parsed; absent when the record field is `None`) — kept OFF `FlowRow` list rows + snapshot summaries. `/v1/completions` still uninstrumented.
