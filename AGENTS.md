...
- Dashboard (gap 05): upstream RESPONSE/ERROR-body capture is a SEPARATE opt-in gate from the debug UI — `LLMCONDUIT_DASHBOARD_CAPTURE_UPSTREAM_RESPONSE=1/true/yes` (env-only, never on `Config`); `--with-debug-ui` alone arms only REQUEST capture. The body lands on the LIVE `FlowRecord.upstream_response` only (`UpstreamResponseBody{bytes,truncated}`), NEVER on `SnapshotFlowSummary` (body-free invariant). Captured at `dispatch_chat_stream`'s terminal-error sites via `capture_response_body` (the SAME capped/redacting `capture_capped_redacted` path — copies the already-read error `String`, never slices the 256 MiB buffer); `truncated = raw.len() > BODY_CAP`. An empty/non-JSON upstream body is recorded as the `[redacted: unparseable body N bytes]` marker (`Some`), distinct from `None` (capture off / no body). `/v1/completions` still uninstrumented.
