 ...
 * StrictMode test inspects this to assert the simulation's `stop()` was CALLED and its
 * tick handler cleared after teardown (findings 7+8). Kept out of the .tsx so the
 * component file exports only the component (react-refresh).
 */
...
export const forceDemoState: {
  ...
  setups: number;
  cleanups: number;
  /** Times `simulation.stop()` was actually invoked (spied in setup) — finding 8. */
  stopCalls: number;
