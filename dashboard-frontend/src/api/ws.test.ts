...
const METRIC_WINDOW = {
  ...
  latency_overflow_count: 0, p50_quality: 'measured' as const, p95_quality: 'partial' as const, p99_quality: 'partial' as const, usage_anomaly_count: 0,
  latency_samples: 7, usage_samples: 7, priced_samples: 7, cost_confidence: 'estimated' as const,
};
const METRICS_SNAP = {
  metrics_seq: 5, generated_at_ms: 5000, instant: METRIC_WINDOW,
};
...
describe('DashboardSocket — time travel (seek/live shadow buffer)', () => {
  ...
  it('live() with a STAGED reconnect snapshot flips to live ATOMICALLY with the snapshot install — never `seeking` with the staged live rows/metrics, then buffered frames replay on top (R6)', () => {
    ...
    expect(st.metrics?.instant.accepted_per_sec).toBe(4.2); // staged snapshot metrics installed
