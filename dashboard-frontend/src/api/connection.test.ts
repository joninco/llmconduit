...
describe('connection — WS-driven REST invalidation (finding 10)', () => {
  ...
  it('a metrics frame invalidates metrics, exact Overview cuts, and topology provider health', () => {
    ...
    const w = {
      ...
      latency_overflow_count: 0, p50_quality: 'partial' as const, p95_quality: 'partial' as const, p99_quality: 'partial' as const, usage_anomaly_count: 0,
    ...
    socket.applyFrame({
      ...
      batch: [{ type: 'metric_tick', generated_at_ms: 1000, instant: w }],
    });
