...
**Status: 6/6 tasks completed (Codex review pending for F1f). F1a–F1e Codex-approved.** Opt-in `turn_capture_dir` writes ONE atomic JSON per turn
...
| F1f | Orphan `.work` sweep (`cleanup_orphan_work_dirs`, mtime-based, wired into `spawn_cleanup`) + rotation confirmed (artifacts + orphan `.work`) + `sole_capture_state` flake fixed (4 F1b tests read the durable `<id>.json` via `wait_for_only_artifact`, not the transient `.work` dir) + docs (FEATURES/AGENTS/README). AC-15,16,17 | ✅ |
