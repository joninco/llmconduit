//! Ported streaming DoS-guard behaviors for the upstream SSE read path (gap
//! G6), adapted from claude-relay's `tests/test_sse.py` (`buffer_overflow`,
//! `exactly_at_limit`, `just_over_limit`, `oversized_unterminated`,
//! `custom_max_buffer`).
//!
//! VERIFY FINDING (recorded here for traceability): llmconduit reads the
//! UPSTREAM SSE response via `response.bytes_stream().eventsource()` in
//! `upstream::stream_success_response`. The `eventsource-stream` 0.2.3 parser
//! (`EventStream::poll_next` / `Utf8Stream::poll_next`) accumulates every byte
//! it receives into an internal `String`/`Vec` buffer and only flushes on an
//! SSE event boundary (a blank line). It does NOT cap that buffer, so a
//! hostile/buggy upstream streaming an oversized or never-terminated frame (no
//! `\n\n`) grows memory without bound. The 256 MiB cap in `http.rs` is the
//! INBOUND request-body limit and does not cover this response-read path. The
//! gap was therefore REAL: we added a configurable per-frame ceiling
//! (`SseFrameGuard`, default 8 MiB, env `LLMCONDUIT_MAX_SSE_FRAME_BYTES`) that
//! rejects an over-cap unterminated frame as a clean `AppError` BEFORE the
//! parser over-accumulates.
//!
//! The pure-guard tests drive `SseFrameGuard` directly (no async, no network)
//! to prove the byte accounting; the wiremock test proves the guard is wired
//! into the real `ReqwestUpstreamClient` upstream-read path end-to-end through
//! `eventsource()`.
...
use llmconduit::upstream::SseFrameGuard;
...
// identical no matter how the bytes are split into chunks (Codex round-2
// Findings 1 & 2). The guard defers any trailing boundary-PREFIX uncharged and
// never re-resets on a boundary it already consumed.
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// REFERENCE-ORACLE differential gate (Codex round-5). Prior gates only asserted
// chunking-INDEPENDENCE (self-consistency: every split agrees with whole). That
// pins nothing ABSOLUTE — a position-specific off-by-one (leading / extra /
// trailing EOL charged to the wrong frame) is self-consistent and slips through.
//
// So we add an independent, obviously-correct, WHOLE-BUFFER reference scanner and
// assert the streaming guard's verdict EQUALS the oracle's for every input. The
// oracle is trivial to get right because it sees the entire buffer at once:
...
//     includes its internal line terminators) — this is the Codex round-6 fix: the
//     old oracle reset on every single EOL and so UNDER-counted multi-line events;
...
/// (its bytes stay inside the frame). Mirrors `next_boundary` in `src/upstream.rs`
/// at end-of-stream (`at_eof`): the whole buffer is present, so a trailing lone CR
/// is a resolved CR EOL.
...
fn reference_oracle_differential_gate_pins_absolute_verdict_every_eol_position() {
    ...
    // whose size INCLUDES its internal single-EOL line terminators. The OLD oracle
    // reset on every single EOL and so UNDER-counted these — a multi-line event whose
    // TOTAL exceeds the cap (but each LINE is under it) was wrongly ACCEPTED. These
    // cases pin the at/over-cap edge for the WHOLE multi-line event and are threaded
    // through every chunking + EOF below, so the guard (which counts the event whole)
    // and the corrected oracle must agree, and the over-cap event must REJECT.
    //
    ...
    for &(inner_label, inner) in &[
        ...
        // cap — well UNDER it — so a per-LINE counter (the old oracle) would accept
        // BOTH; only a per-EVENT counter rejects the over-cap one.
        let line1 = cap / 2;
    ...
    // index 0, so with the round-5 bug it was charged INTO the first frame. With a
...
// --------------------------------------------------------------------------
// End-to-end through the real upstream client + eventsource() parser
// --------------------------------------------------------------------------
