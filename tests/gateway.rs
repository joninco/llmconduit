...
impl MockUpstream {
    ...
    fn set_candidate_models<I, S>(&self, models: I)
    where
...
impl MockVisionClient {
    ...
    async fn requests(&self) -> Vec<llmconduit::vision::VisionRequest> {
        self.requests.lock().await.clone()
    }

    async fn block_on(&self, notify: Arc<Notify>) {
...
async fn cancels_mid_stream_when_client_disconnects() {
    ...
    let gateway = Arc::new(Gateway::new(
        ...
        Arc::new(llmconduit::vision::ImageCache::new(
            100,
...
fn user_message_with_image(text: &str, data_url: &str) -> ResponseItem {
    ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![
...
const TEST_IMAGE_DATA_URL: &str = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAAAAAA=";
