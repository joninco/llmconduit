...
- `responses_to_anthropic/` (`mod.rs` + `reasoning.rs` + `collector.rs` + `conformance.rs`) — `AnthropicStreamConverter` + `AnthropicStreamCollector`. Translates internal Responses SSE into Anthropic `message_start`/`content_block_*`/`message_delta`/`message_stop` — **wire-conformant with vLLM native** (see `ANTHROPIC_STREAM_CONFORMANCE_PLAN.md`): exactly ONE terminal `message_delta` (per-delta output-token counting is bookkeeping-only, never emitted progressively), `thinking` blocks are always signed (real upstream `signature_delta`, else a synthetic `llmconduit-synthetic-v1:<sha256>` marker stripped on ingress), `message_start.input_tokens` carries the early G3 estimate (terminal + final usage carry the real upstream count), and NO `ping`. Maps incomplete max-token stops, handles `response.web_search_results` (additive event) as `server_tool_use` + `web_search_tool_result`. `finalize()` emits a synthetic terminal `message_delta` + `message_stop` if the canonical stream stalls without `response.completed`; an explicit `response.failed` ends the stream at `error` (finalize no-ops). Stream-shape invariants are asserted across all surfaces by `conformance.rs`.
