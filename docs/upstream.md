...
- Raw proxy forwarding uses an allowlist (`should_proxy_request_header`, line 6727). Client `authorization`, `x-api-key`, `api-key`, cookie, proxy-credential, and dashboard/session headers are never forwarded; each upstream uses only its configured credential.
