...
// Reject-path allocation probe (T5 R2): `SseFrameGuard::peak_owned_carry_bytes`
// only maxes `self.carry.len()` AFTER a SUCCESSFUL scan, so it is structurally
// BLIND to a temporary chunk-sized buffer a regressed scanner would allocate and
// drop on the cap-EXCEEDED (`Err`) path before control returns — exactly the old
// `buf = carry ++ chunk` (and the old adapter `Bytes::copy_from_slice`). The
// genuine reject-path guard is the crate's single allocator probe, which observes
// EVERY heap allocation by SIZE regardless of which API made it and records the
// largest one on the arming thread while a guarded region is active. Now shared
// crate-wide (see `crate::test_alloc_probe`) so `dashboard_flow::capture_body` can
// reuse the same single `#[global_allocator]`.
use crate::test_alloc_probe as alloc_probe;
